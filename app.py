...
given_db_name = "vlokired1"
given_db_user = "admin"
given_db_password = "Adi12345678"
given_db_port = "3306"
given_db_endpoint = "database-2.czbjqzo0ad02.us-east-1.rds.amazonaws.com"
...
def signup():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            query = "INSERT INTO vlokired2 (Emailid, Password) VALUES (%s, %s);"
...
def signin():
    ...
    if request.method == 'POST':
        ...
        try:
            table_connect = pymysql.connect(host=given_db_endpoint, user=given_db_user, password=given_db_password, database=given_db_name)
            table_cursor = table_connect.cursor()
            query = " SELECT Password FROM vlokired2 WHERE Emailid = %s"
...
AccessKey = 'AKIAX4AXRHWAOZT6QKEH'
SecreatKey  = 'd5GMU34LfD8bRzIWR6zquJu6UrDEuIRvh1Y41Hse'
...
def upload(file, user):
    try:
        table_connect = pymysql.connect(host=given_db_endpoint, user=given_db_user, password=given_db_password, database=given_db_name)
        table_cursor = table_connect.cursor()
        query = "INSERT INTO vlokired1 (Emailid, File) VALUES (%s, %s);"
...
def fileupload():
    if request.method == 'POST':
        try:
            ...
            S3_Bucket.upload_fileobj(given_file, "vlokired1", fileName)
            expiration_time = 1000
            generatedUrl = S3_Bucket.generate_presigned_url('get_object', Params={'Bucket': 'vlokired1', 'Key': fileName}, ExpiresIn=expiration_time)
            SNS_Client = boto3.client('sns',aws_access_key_id = AccessKey, aws_secret_access_key=SecreatKey, region_name='us-east-1')
